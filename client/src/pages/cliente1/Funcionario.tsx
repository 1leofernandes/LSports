...
const Funcionario: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ensureCss('/cliente1/styles/funcionario.css');
  ...
  const validarUsuario = async () => {
    ...
    try {
      ...
      if (payload.role !== 'funcionario') {
        showToast('Acesso negado. Somente funcionários podem acessar esta página.', 'error');
        navigate('/cliente1/login');
      }
    } catch {
  ...
  const carregarAgendamentos = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_BASE_URL}/agendamentos-funcionario`, {
  ...
  const carregarHistoricoCompleto = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_BASE_URL}/agendamentos-funcionario/historico`, {
        headers: { Authorization: `Bearer ${token}`, 'X-Tenant': getTenant() }
      });
      ...
      <div className="dashboard-header">
        <nav className="navheader">
          <div className="nav-wrapper">
            <a href="#" className="brand-logo">Painel Funcionário</a>
